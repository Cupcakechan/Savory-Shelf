    ...
    <div className="relative group">
      ...
      >
        ...
        <div className="p-4 pr-10">
          <h3 className="font-display font-semibold text-sm text-text leading-snug line-clamp-2 mb-2">{recipe.title}</h3>
          <div className="flex items-center gap-3 flex-wrap">
            {recipe.cookTime && <span className="flex items-center gap-1 text-xs text-muted"><Clock size={11} />{recipe.cookTime}</span>}
            {recipe.servings && <span className="flex items-center gap-1 text-xs text-muted"><Users size={11} />{recipe.servings}</span>}
          </div>
        </div>
      </button>
